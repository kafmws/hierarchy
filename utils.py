...
def reduce_dict(input_dict, average=True):
    ...
    input_dict, after reduction. Always return tensors for consistency of return types.
    """
    names = []
    values = []
...
def gather_dict(input_dict, concat=True):
    """
    Args:
        input_dict (dict): all the values will be gathered
        concat (bool): whether to do contact
    """
    world_size = get_world_size()
    if world_size < 2:
        return input_dict
    with torch.no_grad():
        ...
        for k in sorted(input_dict.keys()):
            ...
            if isinstance(input_dict[k], torch.Tensor):
                ...
                tensor = torch.tensor(input_dict[k], device="cuda")
            gather_list = [tensor.clone() for _ in range(get_world_size())]
            dist.all_gather(gather_list, tensor)
            if concat:
                gather_list = torch.cat(gather_list)
            values.append(gather_list)
...
def main_process_first():
    ...
    if is_dist_avail_and_initialized() and not is_main_process():
        torch.distributed.barrier()
    yield
    if is_dist_avail_and_initialized() and is_main_process():
        torch.distributed.barrier()
