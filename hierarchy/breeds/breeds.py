# almost taken from https://github.com/MadryLab/robustness
# modified and used as https://github.com/MadryLab/BREEDS-Benchmarks
import os
import sys
...
from work.dataset import _ImageNet
...
def setup_breeds(info_dir=os.path.dirname(__file__), url=BREEDS_URL):
...
if __name__ == '__main__':
    setup_breeds()
    ch = ClassHierarchy(info_dir=DIR_PATH)
    
    flat_tree = list(ch.node_to_level.keys())
    print(len(flat_tree))
    leaves = list(ch.leaves_reachable('n00001740'))  # 'n00001740' entity
    print(f'entity tree: {len(leaves)}')
    
    breeds_leaves = [[ch.LEAF_ID_TO_NUM[leaf], leaf, ch.LEAF_ID_TO_NAME[leaf]] for leaf in leaves]
    print(breeds_leaves)
    print()
    
    in1k_class_wnid = set(os.listdir('/data/imagenet_sets/in1k/val'))
    breeds_leaves_wnid = set(ch.leaves_reachable('n00001740'))
    print(f'len(in1k_class_wnid): {len(in1k_class_wnid)} len(breeds_leaves):{len(breeds_leaves)}')
    in1k_class_outof_breeds = in1k_class_wnid.difference(breeds_leaves_wnid)
    for wnid in in1k_class_wnid:
        if wnid not in breeds_leaves_wnid:
            print(ch.LEAF_ID_TO_NAME[wnid])
    # print(in1k_class_wnid)
    print(len(in1k_class_outof_breeds))
    
    # leaves = list(ch.leaves_reachable('n00004258'))  # 'n00004258' living thing, animate thing, 'n00021939' non-living thing
    # print(f'living thing tree: {len(leaves)}')
    # leaves = list(ch.leaves_reachable('n00021939'))  # 'n00021939' non-living thing
    # print(f'non-living thing tree: {len(leaves)}')
    # assert leaves == flat_tree[-len(leaves):]
    # print()
    # print(ch)
    
    left = []
    for n in ch.LEAF_IDS:
        if n in ch.graph.nodes():
            if len(list(ch.graph.predecessors(n))) > 1:
                print(n)
        else:
            left.append(n)
    ...
    # print(list(ch.graph.successors('n00001740')))
    
    # ch.graph['edge'] = {'arrowsize': '0.5'} # 箭头大小
    # ch.graph['node'] = {'shape': 'box'} # 节点形状
    # ch.graph['graph'] = {'splines': 'spline', 'rankdir':'LR'} # 线型与布局方向
    
    # build graphviz tree
    # G = pgv.AGraph(directed=True)
    # for wnid, name in ch.HIER_NODE_NAME.items():
    #     G.add_node(name, color='black')
    
    # name2id = {name: id for id, name in ch.HIER_NODE_NAME.items()}
    # garment = name2id['garment']
    # print(garment)
    # garment_successors = list(ch.graph.successors(garment))
    # print(garment_successors)
    # print(garment in garment_successors)
    ...
    #     todo = []
    #     for w in curr:
    #         for c in ch.graph.successors(w):
    #             G.add_edge(ch.HIER_NODE_NAME[w], ch.HIER_NODE_NAME[c], color='black')
    #             todo.append(c)
    #     todo = set(todo)
    # G.write('breeds.dot')
    # G.draw('file.svg', args='-Gsize=10 -Gratio=1.4', prog='dot')
    
    
    def export_graphviz(dotfile='breeds.dot'):
        
        incls_in_breeds = 0
        
        # output dot file
        with open(dotfile, 'w') as file:
            file.write('strict digraph imagenet_breeds {\n')
            ...
            for wnid, name in ch.HIER_NODE_NAME.items():
                label = name.split(',')[0]
                cls_idx = ch.LEAF_ID_TO_NUM[wnid] if wnid in ch.LEAF_ID_TO_NUM else -1
                nodename = f'{wnid} {name} {cls_idx}'
                wnid2nodename[wnid] = nodename
            ...
            for parent, child in edges:
                parent, child = wnid2nodename[parent], wnid2nodename[child]
                file.write(f'"{parent}" -> "{child}"\n')
            
            file.write('}\n')
        
        os.system(f'dot {dotfile} -T svg -O')
        print(f'imagenet class in breeds: {incls_in_breeds}')
